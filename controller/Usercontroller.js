...
module.exports.Userregister= async(req,res)=>{
   ...
   if(phone)return res.status(200).json('User already Register with this Phone Number--(please use another Phone Number)')


    user=new User(_.pick(req.body,(['firstname','lastname','email','phone','password','confirmPassword'])))
...
try{
    ...
    let user=await User.findOneAndUpdate(req.user,{ $set:req.body},{new:true})
...
try{
    ...
    if(error)return res.status(400).json(error.details[0].message)
        ...
        user=await User.findOneAndUpdate(req.user,{
            $set:{
